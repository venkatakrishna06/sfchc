...
$trimFName= substr($dec_fname, 0, -4);


$bucketName = $_SESSION['reg_uname'].'sfchc';
$filesToDownload = [
    $dec_fname.'/'.$trimFName.'_Encrypted_1.txt',
   $dec_fname.'/'.$trimFName.'_Encrypted_2.txt',
   $dec_fname.'/'.$trimFName.'_Encrypted_3.txt'
];



// Download multiple files from S3 bucket
foreach ($filesToDownload as $objectKey) {
    $downloadPath = 'files/decrypted/'.basename($objectKey);
    try {
        ...
        echo 'Error downloading file: ' . $e->getMessage() . '<br>';
    }
}
...
if ($result->num_rows > 0) {
  ...
  $key3 = $_SESSION['pass3'];
	...
	$dec_filename3 = str_replace(".txt","_Decrypted_3.txt",$filename);
 	$dec_data3 = rsaDecrypt($enc_info3, $key3);    
